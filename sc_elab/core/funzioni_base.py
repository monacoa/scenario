...
def set_data_opt():
    ...
    data_opt['GapFutures'] = 0 # 1 = previous spot rate, 0 = next forward rate
    ...
    data_opt['RegimeOutput'] = 1 # 0 = Semplice, 1 = Composto, 2 = Continuo
...
def compute_z_from_cfr(z_out, tenors, swap_val, times_swap_new, t_swt_n, t_swt_m, times_swap_, df_fix_swap):
    ...
    f1 = 1.0/z_nz*(1.0 - swap_val*sum_z) #% Def. f1 per il calcolo di F
    ...
    fwd_target = optimize.bisect(fwd_fun_0, x_min, x_max, args=(nz, mz, f1, swap_val, times_swap_new))
...
def boot3s_elab_v2(data_opt, data_raw):
    ...
    if (nsw > 0):
        ...
        index_irregular_s  = np.where(index_condition_irr)  # vettore indici date "irregolari" (array, dtype=int64)        
        index_irregular_s  = index_irregular_s[0]         # vettore indici date "irregolari"

        idx_end_irregular=[]
        idx_start_irregular = index_irregular_s
        
        for i in range(0, len(index_irregular_s)-1):
            
                irr_s = index_irregular_s[i]
                irr_e = index_irregular_s[i+1]
